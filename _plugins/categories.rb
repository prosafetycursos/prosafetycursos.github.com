    ...
    def paginate(site, page)
      ...
      (1..pages).each do |num_page|
        ...
        if num_page > 1
          ...
          newpage.pager = pager
          newpage.dir = File.join(page.dir, "/#{page.data['category']}/pagina-#{num_page}")
          site.pages << newpage
        else
          page.pager = pager
        end
  ...
  module Filters
  	...
  	def pager_links(pager)
		...
		if pager['previous_page'] || pager['next_page']
			...
			html = '<div class="pagination float-right"><ul>'
			if pager['previous_page']
				if pager['previous_page'] == 1
					html << "<li class=\"prev\"><a href=\"/#{pager['category']}/\">&nbsp;</a></li>"
				else
					html << "<li class=\"previous\"><a href=\"/#{pager['category']}/pagina-#{pager['previous_page']}\">&nbsp;</a></li>"
				end
			end

                        html << "<li><a href=\"/#{pager['category']}/\">1</a></li>"
                        for count in (2..pager['total_pages'])
                          html << "<li><a href=\"/#{pager['category']}/pagina-#{count}\">#{count}</a></li>"
                        end
			if pager['next_page']
			  html << "<li class=\"next\"><a href=\"/#{pager['category']}/pagina-#{pager['next_page']}\">&nbsp;</a></li>"
			end
